...
const FilterNav: React.FC<Props> = ({ onClick }) => {
  ...
  return (
    <nav className="flex justify-center w-full md:justify-start py-2 z-10 sticky top-0 ">
          ...
          <li className="" key={f.filter}>
            <button
              ...
              className={`cursor-pointer ${
                active === i
                  ? "dark:border-white border-black dark:text-white"
                  : "border-transparent text-gray-600 dark:text-neutral-400"
              } border-b-2   flex items-center justify-center text-sm md:text-base gap-4 hover:text-black hover:dark:text-white text-nowrap duration-300 translate-all ease-in-out py-4 px-2`}
            >
